...
int dbOpen( const char *path ) {
    ...
    addMessage( 2, "Opened database %s\n", path );
    return db;
}
...
struct entry_t *dbGetMusic( const char *dbname ) {
    ...
    addMessage( 1, "Loaded %i titles from the database\n", index-1 );
...
int dbAddTitles( const char *dbname, char *basedir ) {
    ...
    addMessage( 1, "Added %i titles with playcount %i to %s\n", num, mean, dbname );
    dbClose( db );
    return num;
}
...
int dbNameCheck( const char *dbname ) {
    ...
    addMessage(1, "Marked %i titles as removable\n", count );
