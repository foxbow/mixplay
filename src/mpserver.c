...
/* takes a JSON string and converts it into a mpReqInfo structure */
...
static int getHeaders(int sock, mpReqInfo * reqinfo) {
	...
	/* there is data waiting get the request */
	...
	*tagend = 0;
	if (strcasecmp(line, "get") == 0) {
		...
		addMessage(0, "Unsupported method: %s", commdlineata);
		reqinfo->method = none;
	return CL_STP}
			...
			/* all seems good now parse the request.. */
				...
				case getfile:	/* get file */
					...
					if ((strstr(pos, "/ ") == pos) ||
						...
						send(sock,
							 "HTTP/1.0 404 Not Found\015\012\015\012", 26, 0);
						running = CL_STP;
					}
					break;
				case postfile:
					...
					addMessage(MPV, "Part 2\n%s", chunk + strlen(chunk) + 2);
					/* just acknowledge for now... */
					send(sock,
						 "HTTP/1.1 204 No Content\015\012\015\012", 27, 0);
					running = CL_STP;
					break;
				case illegal:
