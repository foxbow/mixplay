...
/**
 * fetches the headers of the current request and fills reqinfo
 */
static int getHeaders(int sock mpReqInfo * reqinfo) {
	struct pollfd pfd;
	int deathcount = 0;
	char line[MP_BLKSIZE] = "";
	size_t len;
	bool reading = true;
	...
	/* there is data waiting go through it line by line */
	while (reading) {
		char *pos;

		len = readline(line, MP_BLKSIZE, sock);
		if (len == 0)
			break;
		if (pos ==)
			}
 ...
 */
			...
			char *manifest = NULL;
			mpReqInfo reqInfo = { 0, NULL, 0, none, NULL };
			...
			do {
				...
				if (pfd.revents & POLLIN) {
					...
					/* all seems good now parse the request.. */
						...
						method = none;
						// toLower(commdata);
						addMessage(MPV + 3, "Size %li/%li", retval, commsize);
						addMessage(MPV + 3, "%s", commdata);
						end = strchr(commdata, ' ');
						if (end == NULL) {
							addMessage(MPV + 1, "Malformed HTTP: %s",
									   commdata);
							method = illegal;
						}
						else {
							*end = 0;
							...
							if (strcasecmp(commdata, "get") == 0) {
								...
								addMessage(0, "Unsupported method: %s",
										   commdata);
								method = none;
							}
						}
						if ((int) method > 0) {
							...
							/* TODO: feels like this is not adding value but just making things worse 
							 * see line 452 */
							if (end == NULL) {
								addMessage(MPV + 1, "Malformed request %s",
										   pos);
								method = illegal;
							}
							/* control command */
							...
							/* everything else is treated like a GET <path> or upload */
							else {
								if (method == get) {
									method = getfile;
									if (running & CL_RUN) {
						...
						case get:	/* GET mpcmd */
							if (strcmp(pos, "/status") == 0) {
								...
								addMessage(MPV + 3, "Statusrequest: %i",
										   fullstat);
							}
								...
								if (strstr(pos, "info ") == pos) {
									...
									send(sock,
										 "HTTP/1.0 404 Not Found\015\012\015\012",
										 25, 0);
									running = CL_STP;
								}
							}
						...
						case post:	/* POST */
							if (strstr(pos, "/cmd") == pos) {
								...
								send(sock,
									 "HTTP/1.0 406 Not Acceptable\015\012\015\012",
									 31, 0);
								running = CL_STP;
							}
							break;
						case getfile:	/* get file */
							...
							break;
						case postfile:
							chunk = pos + strlen(pos) + 2;
							addMessage(MPV, "Got a post!\n%s", chunk);
							addMessage(MPV, "Part 2\n%s",
									   chunk + strlen(chunk) + 2);
							/* just acknowledge for now... */
							send(sock,
								 "HTTP/1.1 204 No Content\015\012\015\012", 27,
								 0);
							running = CL_STP;
							break;
						case illegal:
							...
							send(sock,
								 "HTTP/1.0 405 Method Not Allowed\015\012\015\012",
								 35, 0);
							running = CL_STP;
							break;
						case filenotfound:	/* generic file not found */
							send(sock,
							...
							break;
						default:
							addMessage(0, "Unknown method %i!", (int) method);
							running = CL_STP;
							break;
						}		/* switch(method) */
					}			/* switch(retval) */
				...
				if (running != CL_STP) {
					...
					case req_update:	/* get update */
						...
						if (jsonLine != NULL) {
							...
							while ((ssize_t)
								   (strlen(jsonLine) + strlen(commdata) + 8)
								   > commsize) {
								commsize += MP_BLKSIZE;
								commdata =
									(char *) frealloc(commdata, commsize);
							}
					...
					if (len > 0) {
						...
						while (sent < len) {
							msglen =
								send(sock, commdata + sent, len - sent, 0);
							if (msglen > 0) {
								sent += msglen;
							}
							else {
		...
		static void *mpserver(void *arg) {
			...
			while (control->status != mpc_quit) {
				if (poll(&pfd, 1, 250) > 0) {
					...
					if (pthread_create
						(&pid, NULL, clientHandler,
						 (void *) (long) client_sock) < 0) {
						addMessage(0,
								   "Could not create client handler thread!");
					}
				}
			}
 ...
 */
		int32_t startServer() {
			...
			size_t bmlen =
				static_bookmarklet_js_len + strlen(host) + strlen(port);
			...
			pthread_create(&control->stid, NULL, mpserver,
						   (void *) (long) mainsocket);
