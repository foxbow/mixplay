...
static int g_activity( void *text ) {
    if ( gldata(mpcontrol)->widgets->mp_popup != NULL ) {
        gtk_window_set_title( GTK_WINDOW( gldata(mpcontrol)->widgets->mp_popup ), ( char * )text );
        gtk_widget_queue_draw( gldata(mpcontrol)->widgets->mp_popup );
    }
    else if( gldata(mpcontrol)->widgets->album_current != NULL ) {
        gtk_label_set_text( GTK_LABEL( gldata(mpcontrol)->widgets->album_current ),
                            text );
        gtk_widget_queue_draw( gldata(mpcontrol)->widgets->mixplay_main );
    }
...
static int g_progressLog( void *line ) {
	...
	scrollAdd( gldata(mpcontrol)->log, line, MP_LOGLEN );
	if( NULL != gldata(mpcontrol)->widgets->mp_popup ) {
		gtk_message_dialog_format_secondary_text( GTK_MESSAGE_DIALOG( gldata(mpcontrol)->widgets->mp_popup ),
				"%s", gldata(mpcontrol)->log );
		gtk_widget_queue_draw( gldata(mpcontrol)->widgets->mp_popup );
	}
...
static void cb_progressClose( GtkDialog *dialog, gint res, gpointer data ) {
    ...
    gldata(mpcontrol)->widgets->mp_popup=NULL;
    gldata(mpcontrol)->log[0]='\0';
    pthread_mutex_unlock( &msglock );
}
...
static int g_progressStart( void *line ) {
    if( NULL == gldata(mpcontrol)->widgets->mp_popup ) {
        gldata(mpcontrol)->widgets->mp_popup = gtk_message_dialog_new ( GTK_WINDOW( gldata(mpcontrol)->widgets->mixplay_main ),
                                       GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE,
                                       "%s", ( char * )line );
        gtk_dialog_add_button( GTK_DIALOG( gldata(mpcontrol)->widgets->mp_popup ), "OK", 1 );
        g_signal_connect_swapped ( gldata(mpcontrol)->widgets->mp_popup, "response",
                                   G_CALLBACK( cb_progressClose ),
                                   gldata(mpcontrol)->widgets->mp_popup );
    }
    gtk_widget_set_sensitive( gldata(mpcontrol)->widgets->mp_popup, FALSE );
    gtk_widget_show_all( gldata(mpcontrol)->widgets->mp_popup );
...
static int g_progressEnd( void *line ) {
    gtk_widget_set_sensitive( gldata(mpcontrol)->widgets->mp_popup, TRUE );
...
static int g_updateUI( void *data ) {
    ...
    gtk_widget_set_visible( gldata(control)->widgets->button_next, ( !control->playstream ) );
    gtk_widget_set_sensitive( gldata(control)->widgets->button_prev, !( control->playstream ) );
    gtk_widget_set_visible( gldata(control)->widgets->progress, !( control->playstream ) );
    gtk_widget_set_visible( gldata(control)->widgets->played, !( control->playstream ) );
    gtk_widget_set_visible( gldata(control)->widgets->remain, !( control->playstream ) );

    if( ( NULL != control->current ) && ( 0 != strlen( control->current->path ) ) ) {
        usedb=( control->root->key )?TRUE:FALSE;
        /* These depend on a database */
        gtk_widget_set_visible( gldata(control)->widgets->button_fav, usedb );

        gtk_label_set_text( GTK_LABEL( gldata(control)->widgets->title_current ),
                            control->current->title );

        if( control->root->key != 0  ) {
        	infoLine( buff, control->current->plprev, MAXPATHLEN );
            gtk_widget_set_tooltip_text( gldata(control)->widgets->button_prev, buff );
        	infoLine( buff, control->current, MAXPATHLEN );
            gtk_widget_set_tooltip_text( gldata(control)->widgets->title_current, buff );
        }
        else {
            gtk_widget_set_tooltip_text( gldata(control)->widgets->title_current, control->current->path );
            gtk_widget_set_tooltip_text( gldata(control)->widgets->button_prev, NULL );
        }
        ...
        gtk_widget_set_tooltip_text( gldata(control)->widgets->button_next, buff );

        gtk_widget_set_sensitive( gldata(control)->widgets->title_current, ( control->status == mpc_play ) );
        gtk_label_set_text( GTK_LABEL( gldata(control)->widgets->artist_current ),
                            control->current->artist );
        gtk_label_set_text( GTK_LABEL( gldata(control)->widgets->album_current ),
                            control->current->album );
        gtk_label_set_text( GTK_LABEL( gldata(control)->widgets->genre_current ),
                            control->current->genre );
        setButtonLabel( gldata(control)->widgets->button_prev, control->current->plprev->display );

        if( gldata(control)->debug > 1 ) {
            sprintf( buff, "%2i %s", control->current->skipcount, control->current->plnext->display );
            setButtonLabel( gldata(control)->widgets->button_next, buff );
        }
        else {
            setButtonLabel( gldata(control)->widgets->button_next, control->current->plnext->display );
        }

        gtk_widget_set_sensitive( gldata(control)->widgets->button_fav, ( !( control->current->flags & MP_FAV ) ) );

        if( 0 == control->active ) {
           	if( control->playstream ) {
           	    gtk_widget_set_visible( gldata(control)->widgets->remain, 0 );
        		setButtonLabel( gldata(control)->widgets->button_profile, "Add Stream" );
           	}
           	else {
           	    gtk_widget_set_visible( gldata(control)->widgets->remain, -1 );
        		setButtonLabel( gldata(control)->widgets->button_profile, "Profile.." );
           	}
        }
        else {
    	    gtk_widget_set_visible( gldata(control)->widgets->remain, -1 );
    	    snprintf( buff, MAXPATHLEN, "Playing \n%s", (control->active < 0)?control->sname[-control->active-1]:control->profile[control->active-1] );
        	setButtonLabel( gldata(control)->widgets->button_profile, buff );
        }

        gtk_window_set_title ( GTK_WINDOW( gldata(control)->widgets->mixplay_main ),
                               control->current->display );
    }


    gtk_label_set_text( GTK_LABEL( gldata(control)->widgets->played ),
                        control->playtime );
    gtk_label_set_text( GTK_LABEL( gldata(control)->widgets->remain ),
                        control->remtime );
    gtk_progress_bar_set_fraction( GTK_PROGRESS_BAR( gldata(control)->widgets->progress ),
